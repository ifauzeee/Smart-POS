...
function ProductFormModal({ isOpen, onClose, onSave, product }) {
  ...
  useEffect(() => {
    // Jika ada produk (mode edit), isi form dengan datanya. Jika tidak (mode tambah), form kosong.
    setFormData(product || { name: '', category: '', price: '', stock: '', image_url: '' });
  }, [product, isOpen]);
        ...
        <ModalBackdrop initial="hidden" animate="visible" exit="hidden" variants={backdropVariants}>
          <ModalContainer variants={modalVariants}>
            <form onSubmit={handleSubmit}>
              <ModalHeader>
                <ModalTitle>{product ? 'Edit Produk' : 'Tambah Produk Baru'}</ModalTitle>
